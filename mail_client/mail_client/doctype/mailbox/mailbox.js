...
frappe.ui.form.on("Mailbox", {
	...
	set_queries(frm) {
		...
		frm.set_query("user", () => ({
			query: "mail_client.utils.query.get_users_with_mailbox_user_role",
			filters: {
				enabled: 1,
				role: "Mailbox User",
			},
	...
	delete_incoming_mails(frm) {
		frappe.call({
			method: "mail_client.mail_client.doctype.mailbox.mailbox.delete_incoming_mails",
			args: {
	...
	delete_outgoing_mails(frm) {
		frappe.call({
			method: "mail_client.mail_client.doctype.mailbox.mailbox.delete_outgoing_mails",
			args: {
				mailbox: frm.doc.name,
			},
			freeze: true,
