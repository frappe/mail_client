...
from mail_client.mail_server import MailServerAuthAPI


class MailClientSettings(Document):
...
def validate_mail_client_settings() -> None:
	"""Validates the mandatory fields in the Mail Client Settings."""

	client_settings = frappe.get_doc("Mail Client Settings")
	mandatory_fields = [
		"mail_server_host",
		"mail_server_api_key",
	...
	for field in mandatory_fields:
		if not client_settings.get(field):
			field_label = frappe.get_meta("Mail Client Settings").get_label(field)
			frappe.throw(
				_("Please set the {0} in the Mail Client Settings.").format(frappe.bold(field_label))
			)
